...
async function getUsers(){
  ...
  const res = await fetch("http://192.168.63.56/API/users.php")
  // console.log(res)
...
function createUserList(usersList) {
  ...
  usersList.forEach(user => {
    ...
    <td>${user.Mail}</td>
    <td>${user.cat}</td>
    <td><a href="../php/update_art.php?id=${user.id}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen-fill" viewBox="0 0 16 16">
